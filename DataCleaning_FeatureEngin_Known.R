...
# read data frame
orders = read.csv(file="BADS_WS1718_known.csv", header=TRUE, sep=",", stringsAsFactors = FALSE)

#load helperfunction
source("1.1-helperfuncs.r")

############################################################################
### Data Cleaning and Feature Engineering ##################################
############################################################################

# clean dataset and do feature engineering
orders = CleanDataset(orders)

# factorize return
orders$return = as.factor(orders$return)

# determine if a customer_id returned before
...
# load package for imputation
library(Hmisc)

# impute NAs and remove wrong spelled variable
...
# remove dataframes
rm(again) # Wenn again gelöscht wird, kann der unknown datensatz nicht wissen, welche id bereits retoured hat
rm(orders)
...
save.image(file="Orders_known_updated.RData")